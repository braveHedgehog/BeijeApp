...
interface Props {
  ...
  expanded: boolean;
  onDotPress?: (note?: string, date?: string) => void;
}
...
// Dairenin merkezden ölçülen iç yarıçapı; strokeWidth'ın yarısı içeride
...
const CycleArc: React.FC<Props> = ({
  ...
  // Her nokta için, dairenin sınırlarına taşmaması adına
  ...
  return (
    <View style={styles.container}>
      ...
      {menstrationDays.map((day, index) => {
        ...
        // Nokta boyutu: expanded modda büyük, normalde küçük
        const dotSize = isBigDot ? BIG_DOT_SIZE : DOT_SIZE;

        // Noktaların merkez koordinatları; effectiveRadius'ı kullanıyoruz
        ...
        return (
          <TouchableOpacity
            ...
            onPress={() => onDotPress && onDotPress(day.note) && onDotPress(day.date)}
            activeOpacity={0.8}
          />
        );
      })}
